...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        image = image.resize(self.downsize)
        depth = depth.resize(self.downsize, Image.NEAREST)
...
class Scale_seq(object):
    ...
    def __call__(self, sample):
        image, depth = sample['image'], sample['depth']
        next_image, next_depth = sample['next_image'], sample['next_depth']

        image = self.changeScale(image, self.size)
        depth = self.changeScale(depth, self.size,Image.NEAREST)
        
        next_image = self.changeScale(next_image, self.size)
        next_depth = self.changeScale(next_depth, self.size,Image.NEAREST)
 
        return {'image': image, 'depth': depth, 'next_image': next_image, 'next_depth': next_depth}
...
class RandomHorizontalFlip_seq(object):
    ...
    def __call__(self, sample):
        ...
        if not _is_pil_image(next_image):
            raise TypeError(
                'img should be PIL Image. Got {}'.format(type(img)))
        if not _is_pil_image(next_depth):
            raise TypeError(
                'img should be PIL Image. Got {}'.format(type(depth)))                        
        ...
        if random.random() < 0.5:
            ...
            next_depth = next_depth.transpose(Image.FLIP_LEFT_RIGHT)

        return {'image': image, 'depth': depth, 'next_image': next_image, 'next_depth': next_depth}



class RandomRotate_seq(object):
    """Random rotation of the image from -angle to angle (in degrees)
    This is useful for dataAugmentation, especially for geometric problems such as FlowEstimation
    angle: max angle of the rotation
    interpolation order: Default: 2 (bilinear)
    ...
    def __call__(self, sample):
        ...
        next_image = ndimage.interpolation.rotate(
            next_image, angle1, reshape=self.reshape, order=self.order)
        next_depth = ndimage.interpolation.rotate(
            next_depth, angle1, reshape=self.reshape, order=self.order)

        image = Image.fromarray(image)
        depth = Image.fromarray(depth)
        next_image = Image.fromarray(next_image)
        next_depth = Image.fromarray(next_depth)
...
class CenterCrop_seq(object):
    ...
    def __call__(self, sample):
        ...
        image = self.centerCrop(image, self.size_image)
        depth = self.centerCrop(depth, self.size_image)
        next_image = self.centerCrop(next_image, self.size_image)
        next_depth = self.centerCrop(next_depth, self.size_image)

        ow, oh = self.size_depth
        depth = depth.resize((ow, oh))
        next_depth = next_depth.resize((ow, oh))
...
class Resize_seq(object):
    ...
    def __call__(self, sample):
        ...
        next_image = next_image.resize(self.downsize)
        next_depth = next_depth.resize(self.downsize, Image.NEAREST)
 
        return {'image': image, 'depth': depth, 'next_image': next_image, 'next_depth': next_depth}


class ToTensor_seq(object):
    """Convert a ``PIL.Image`` or ``numpy.ndarray`` to tensor.
    Converts a PIL.Image or numpy.ndarray (H x W x C) in the range
    [0, 255] to a torch.FloatTensor of shape (C x H x W) in the range [0.0, 1.0].
    """
    ...
    def __call__(self, sample):
        ...
        if self.is_test:
            ...
            depth = self.to_tensor(depth).float()
            next_depth = self.to_tensor(next_depth).float()            
            return {'image': image, 'depth': depth, 'next_image': next_image, 'next_depth': next_depth}
